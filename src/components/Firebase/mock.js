class FirebaseMock {
    ...
    updateTask = (task) => {
        let foundIndex = this.findTask(task.id);

        if (foundIndex !== -1) {
            this.tasks[foundIndex] = Object.assign({}, task);
            this.fireTaskListUpdate();
        }
    }

    newTask = (newtopic, newaction, duedate) => {
        this.tasks.push({ id: (this.idCounter++).toString(), topic: newtopic, action: newaction, due: duedate });
